...
export class CssVarDirective implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // let 
    let delimiter = ':get:';
    if (!this.cssVar.includes(delimiter) && this.cssVar.includes(':')) delimiter = ':'; 
  ...
  private getRatio() {
    let aspect = [...this.native.classList as any].find(a => a.indexOf('aspect-') > -1)??'aspect-1x1';
  ...
  private getValue() {
    ...
    switch (this.attribute) {
      case 'aspect-height':
        value = ratioCalculator(this.getRatio(), this.native.getBoundingClientRect?.()?.width);
        break;

      case 'aspect-width':
        value = ratioCalculator(this.getRatio(), this.native.getBoundingClientRect?.().height);
        break;
    ...
    if (this.native?.hasAttribute('rect')) 
      value = (this.native.getBoundingClientRect() as any)?.[this.attribute];
