...
export function encapsulateFilesInFormData(files: FileList, key: string): FormData {
...
export const responsiveImageMediaQueryRegex: RegExp = /(\([max|min|orientation].+\))\s(.+)/i;
...
export function reduceProperties(object: any, brackets: ReduceProperty[], matchingAttribute?: string): any {
...
export function elementHeightInViewport(item: HTMLElement, parent: HTMLElement): number {
...
export function queryVideoAspectRatio(url: string, dimensions?: boolean): Promise<{
    ratio: string;
    width: number,
    height: number
}> {
...
export function isChinaOrigin(): boolean {
    return window.location.host.indexOf('ecco.cn') !== -1 || window.location.host.indexOf('cn.eccocp') !== -1
}
...
export function getCss(hyphenatedAttr: any, element = document.documentElement) {
    try {
        let source = getComputedStyle(element);
        if (hyphenatedAttr?.length) {
            return getComputedStyle(element)[camelizeHyphenated(hyphenatedAttr)];
        }
        return source;
    } catch (e) {
